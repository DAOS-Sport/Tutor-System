...
- 2026-07-16：試上課程補完（PRP 計劃 P1-P5，凍結區改動經使用者批准 PLAN §10 後實作）：
  ...
  - **凍結區（P4，PLAN §10 批准）**：MyCoursesPage 試上卡加「續報一般課程」按鈕（active 附加、completed 卡=續報+查看上課紀錄；`/mine` 已回 coach.id/venue.id/course_type，server 零改動）；CourseCard「試上・單堂」徽章（非凍結）；`admin/checkins.js` recSql＋`admin/sessions.js` `REAL_SESSIONS_SELECT`（含兩處 UNION 腿補 `FALSE`）帶出 `is_experience_course` → CheckinPage/SessionsPage「試上」徽章。
  - **政策項（PRP §8-6，使用者回貼 PLAN「繼續」批准）**：試上課期效期 365→**30 天**（僅 `order_kind='trial'`，一般報名 365×期數不變，`ensureSoloCoursePeriod` solo 分支）；期末評鑑 cron（🧊 `cron/index.js` due 查詢）加 `is_experience_course=FALSE` 排除試上；上課記錄頁試上標記（🧊 `courses.js /lessons` 加 `is_experience_course`＋`MyLessonsPage` 徽章，§10-7）。**未做（維持開放題）**：櫃台手動建試上單（§8-5，後端手動建檔不寫 order_kind、單筆 reconcile 會放大成 6，要支援需另補）、WS reconcile 事件（§8-8 建議不做）。
  ...
  - **（驗收修正）選教練頁試上價**：試上流程（`trial=1`）的 CoachCard 原顯示一般每期價（如 NT$9,000）→ 改顯示試上單堂價，與後端 `calculateTrialPrice` 同語意：F-A07 `trial_price` 有設＝每人固定價（不吃教練係數、含「待分配」卡）；未設＝單期價×係數÷每期堂數推算；標籤「/ 試上 1 堂」。`CoachListPage` 傳入 base-price 完整回應（trial_price/sessions_per_period）。
  - **（驗收修正）試上多學員／多堂放行**：原 `TRIAL_SINGLE_ORDER_REQUIRED` 擋「多位學員或多期」→ 使用者裁定不該擋。改為試上**以每人每堂計價**、依「學員 × 堂數」拆分子訂單（每筆＝1 學員 1 堂 `total_sessions=1`），對帳後逐筆開通**獨立** 1 堂體驗課期（`ensureSoloCoursePeriod` 對 trial 本就排除共班分支，下游對帳/簽到/退費零改動）；課型人數上限檢查（`STUDENT_COUNT_EXCEEDS_COURSE_TYPE`）對試上不適用（不共班）。前端：報名頁試上模式開放多選學員（上限＝名下學員數）與堂數下拉（1–6 堂、「N 堂」語意）、切換試上不再清空選擇、費用明細顯示「每人單堂 × 數量」；`useEnrollmentPricing` 試上小計 × 學員 × 堂數。e2e trial_full_chain 補「2 學員 × 2 堂 → 4 子訂單 → 4 獨立課期」斷言。多代理審查後加固：①學員 id 正規化小寫＋UUID_RE 嚴格驗證＋`STUDENT_RESOLUTION_MISMATCH` 縱深防禦（既有缺口——非正規 uuid 形式過 DB 驗證但 JS Map miss → 子訂單數與付款單金額脫鉤的幽靈單，多學員放行後影響面變大）；②試上多選切回一般報名時超額學員自動裁到 courseType 上限（避免送出鈕永遠灰的死路）。回歸：trial_full_chain（含 id 正規化新斷言）/enrollment_idempotency/family_shared_period/checkout_multi_student_periods/path_a_purchase 全過，LIFF 已重建。
