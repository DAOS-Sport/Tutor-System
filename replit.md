...
- 2026-07-03：Z01/Z02/Z03 同步缺陷修復（上線前資料清洗後遺症排除）：
  ...
  - **Z03 UID 自癒（只補完整記錄）**：Ragic 記錄「只缺家教系統uid、其餘必填齊全」且本地同號家長已綁真實 LINE UID（資料清洗重建記錄時 UID 被丟掉的典型後遺症）→ pull 自動把本地 UID 回寫 Ragic，該筆立即走完成路徑進 parents/students + Z03 resolved，不再每晚回落 Z03 卡死。安全邊界：Ragic 端 UID 為空/哨兵值才寫、先查 UID 未掛在其他 Ragic 記錄；任一必填仍缺就留在 Z03，不塞進本地鏡像。pull 回傳新增 `healed_uid` 計數。
  ...
  - 注意：production 需重新 Publish 才會帶上以上程式修復；發布後隔天 01:30 pull 會自動回填 venue 與自癒 UID，或可在後台 Ragic 狀態頁手動觸發。已驗證：LIFF build PASS、後端語法檢查 PASS、`smoke:ragic-auth` 9/9 PASS（write smoke disabled）。
