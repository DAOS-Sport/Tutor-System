...
**原則：Ragic = 人事唯一權威。** 各端與 H01 的關係：
- **教練 LIFF**：零 Ragic 寫入（實測稽核：寫入函式只被家長流程 `auth.js`/`parents.js`/`groupOrders.js` 呼叫）。教練只能改自我介紹（bio/介紹圖，存本地 DB，不在 H01 範圍）。
...
為什麼不做 admin→H01 寫回（曾實作後依政策移除，技術限制實測紀錄）：
- H01 新建必填欄位 14 個（國籍/身分證字號/直屬主管/聘雇類別/招聘來源/到職日期/系統群組/HR專員…）全是 HR 專屬資料，系統無法代建。
- Ragic 更新時整筆重驗必填：紀錄缺任一必填（如「教學項目」「400Line訊息」）→ API 完全無法更新該筆。
- 實機驗證過的 Field ID（查考用）：`3000933`=姓名、`3001424`=手機（顯示名「手機」；`3000941` 手機（個人）無資料）、`3000940`=電子郵件信箱、`3000937`=部門（多選）、`3000935`=員工編號。LINE UID（`1003633`）由管理員在 Ragic 維護。
