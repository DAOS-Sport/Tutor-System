...
## 15. 實作後的調整（擁有者 2026-09-23 晚間定案；與上文不同時以本節為準）

1. **成員的唯一鍵是 LINE userId**（跟主帳號同一套）：加入時記下當下綁定的 userId；移除＝解綁 userId
   （清成 NULL，稽核只留雜湊）。主帳號換過 LINE 的成員失效，櫃台再加一次＝重新綁定。
2. **預先登記的認領採最簡單版（A）**：要先綁 LINE，而且註冊時填的孩子身分證＋生日要跟家裡的孩子相符才加入；
   認領入口在個人頁（每條註冊路徑都會經過）。
3. **排程推播不發全家**：上課前提醒、到期提醒、評鑑邀請維持只發給原本的家長，排程程式不動。
4. **櫃台加家人有三條路**：邀請連結（一次性、7 天有效、可作廢、同一家庭同時最多 5 條）、
   手機＋姓名直接添加（姓名對得上才帶出 LINE UID，UID 不必手打）、預先登記。
5. **家長端邀請**：擁有者、或還沒有家庭的家長可以在個人頁「我的家庭」產生邀請連結；
   平常只顯示按鈕，點了才產生（後台同樣）。還沒有家庭的人產生連結時只先建「準備中的家庭」
   （沒有成員），有人用連結加入時才成立、邀請人當擁有者 —— 避免只按了按鈕就變成一人家庭，
   之後加入不了配偶的家庭。自己的連結不能自己用；邀請人已加入別的家庭時，他之前的連結失效。
6. **凍結功能拿掉，改成解綁**：擁有者可以在個人頁解綁加入的家人；擁有者本人（原本辦理學員的那位）
   不能被解綁，只能編輯；轉移擁有者由櫃台（admin）處理。
7. **§14 申請加入改成「孩子的名字＋對方家長的手機」**：對方家長在個人頁同意才生效；對方在家庭裡但不是
   擁有者時由擁有者決定；櫃台仍可在後台核准或退回（備援）。不符一律同一句通用訊息、照樣記次數（24 小時 5 次）。
8. **新增學員時直接綁定**：身分證＋姓名都對得上別的帳號下的有效孩子 → 直接加入那位孩子所在的家庭
   （還沒有家庭就以孩子的家長為擁有者成立），不另建學員、不碰 Ragic；姓名對不上照舊擋下，改走申請。
   起因是案例家庭：孩子在 Ragic 掛在媽媽帳號，爸爸新增同一個身分證一直被擋。只在開關有開時生效。
9. **重複學員一律整批處理**：邀請加入、核准申請、櫃台添加成員、新增學員綁定，都把新成員名下所有跟家人
   重複的孩子依 §9 處理（原本核准申請只處理申請單上那一位）。
10. **被移出家庭不自動恢復**：名下被合併停用的孩子不會自動恢復，需要時由櫃台在 Z01 啟用。
11. **範圍**：註冊流程仍要填孩子；個人頁的「我的學員」仍只列自己名下（家人的孩子在「我的家庭」唯讀顯示）。
    以家庭計的優惠與推薦只有：每期上限以家庭合計、TRIAL50 的推薦人是家人不算新客、同家庭互推不發獎勵券；
    團購同一家庭同一團只算一戶。
12. **時間**：所有顯示時間一律台北時間。

**已知風險**：知道孩子姓名＋身分證的人可以直接綁進那個家庭（第 8 點，擁有者接受）；家庭通知目前預設關閉，
對方家長不會收到「某某已加入」的 LINE。緩解：開關只開給試點手機；擁有者與櫃台都能解綁。
