...
family_join_requests（新，見 §14）
  id, applicant_parent_id, target_student_id, duplicate_student_id（申請人名下的重複學員，可空）,
  relationship, note, status CHECK IN ('pending','approved','rejected','cancelled'),
  reviewed_by, reviewed_at, reject_reason, created_at
  -- 同一申請人同時只能有一筆 pending（部分唯一索引）
...
3. **「家庭申請與建議」按鈕**（頁面上方，顯示待處理數）：分兩個分頁——
   - **申請**：家長從手機送來的合併申請（§14）。畫面並排顯示申請人、目標孩子與其所屬家長、
     申請人名下的重複學員（含課程數、生日是否相符）。核准 → 自動建家庭或加入既有家庭，
     重複學員依 §9 規則處理；退回要填原因。
   - **建議**：系統偵測到「同一個身分證字號掛在不同帳號」但還沒人申請的孩子（目前 24 位）。
     點一筆 → 確認視窗預填擁有者（Ragic 上的家長）、成員、關係、要停用的重複學員 → 櫃台確認才建立。
   系統只建議，不自動建立；兩個分頁最後都走同一個核准動作。
...
- 個人頁「我的家庭」卡片：已在家庭 → 成員與關係、孩子依所屬家長分組、「退出家庭」；
  不在家庭 → 「申請加入家庭」（§14）；申請中 → 顯示「審核中」與「取消申請」。
- 個人頁頂端提示（比照現有「部分學員資料尚未完整」黃色提示）：名下有重複學員的帳號顯示
  「你登記的孩子跟另一個家長帳號是同一人。如果你們是一家人，可以申請合併到同一個家庭。」→「申請合併」。
- 我的課程、帳單：全家的，標示購買人。
- 簽到：全家的孩子都有簽到鈕。
- 預約：全家的孩子都能選教練已開放的時段。
- 團購（第二階段）：家人已在團內時，邀請頁顯示「您的家人已加入此團」，不再給加入鈕。
- 新增學員時「此身分證字號已有學員資料」的紅字，改成說明＋按鈕：「這位孩子已經登記在另一個
  家長帳號下。如果你們是一家人，可以申請加入同一個家庭，櫃台確認後就能一起查看、繳費、簽到。」
  →「申請加入家庭」（剛填的身分證與生日直接帶入申請，不建立第二份學員）。

## 9. 既有 24 位孩子的搬遷

兩條路併行，都是**逐家確認**，不做批次：家長自己從手機申請合併（§14），或櫃台從
「家庭申請與建議」的「建議」分頁主動處理。核准時的處理規則相同：
...
- **合併申請（§14）**：身分證＋生日不符只回通用訊息、不洩漏對方資料；每日次數上限；同一人只能有
  一筆審核中；核准後重複學員依 §9 處理且不再出現在每晚 Ragic 寫回失敗名單；退回與取消不改任何資料。
...
第一版的四題已由擁有者回覆（2026-09-23，見決策 5～8）；以下兩點解讀擁有者已確認沒問題，
並補充「家庭可以共同查看團報」（已列入 §5 第一階段）：
...
## 14. 家長自助申請合併家庭（處理孤兒資料）

**孤兒資料是什麼**：同一個孩子被第二位家人再登記一次，系統裡多出一份學員資料。註冊流程
（`z03IdentityClaim.completeTrueUidRegistration` → `parentSync.upsertLocalStudents`）只在同一位家長
底下比對孩子，不查其他帳號的身分證；「個人頁新增學員」才有擋（就是現在那句紅字）。所以第二位家人
註冊時就會多一份。
正式庫 2026-09-23：每晚寫回 Ragic 失敗的 61 筆學員裡，**31 筆就是這種**（Ragic 回「學員已連結
其他資料」），另外 30 筆是缺生日或身分證。

**一條規則，所有入口都一樣（不是特例）**

1. 一個孩子只有一份學員資料，掛在一位家長（Ragic 上的家長）底下。
2. 其他家人要一起管理，一律透過「家庭」，而且要櫃台核准。
3. 核准後，成員跟擁有者用同一套規則操作（§2）；不因為是誰申請的而有例外。

**流程**

1. 家長照常註冊（註冊流程不改）。
2. 在個人頁送出合併申請，三個入口：
   - 名下已有重複學員 → 個人頁頂端提示「申請合併」（系統已知道是哪個孩子，只要選關係）。
   - 新增學員被紅字擋下 → 紅字改成「申請加入家庭」按鈕，剛填的身分證與生日直接帶入。
   - 「我的家庭」卡片 →「申請加入家庭」，自己填孩子的身分證字號＋生日。
3. 申請內容：目標孩子（身分證＋生日都要相符）、你和孩子的關係、備註。不相符只回
   「資料不符，請確認或洽櫃台」，不透露任何對方資料；每個帳號每天最多 5 次、同時只能有一筆審核中。
4. 送出後：申請人看到「審核中」；孩子的所屬家長收到 LINE：「○○（爸爸）申請加入您的家庭，
   櫃台確認後生效；如果不認識這位申請人，請聯絡櫃台。」
5. 櫃台在「家庭申請與建議」審核（§7）：
   - 核准 → 孩子的所屬家長還沒有家庭就以他為擁有者建立；申請人加入為成員。
   - 申請人名下的重複學員依 §9 處理：沒課就停用（每晚寫回失敗隨之消失）；有課保留到課程結束。
   - 退回要填原因，通知申請人；什麼資料都不改。
6. 核准後申請人立刻可以查看、繳費、簽到、預約、查看團報；第二階段起也能幫忙報課。

**幫忙報課（第二階段）的規則**

- 可以替家庭裡任何一個孩子報課；購買人＝下單的成員，發票與發票 Email 都給他。
- 孩子的所屬家長不變，Ragic 不動。
- 新客、試上、推薦資格以家庭為單位；同一家庭在同一團只算一戶（決策 6）。

**離開家庭**：成員可以自己退出，櫃台也可以移除；立即失去看家庭孩子的權限。自己付過的訂單
仍在自己名下可查。擁有者不能自己退出，要先由櫃台轉移擁有者。

**會建立學員的路徑（12 處）**：本節只改個人頁新增學員（`parents.js` POST /me/students）。
註冊、團購參團時新增學員等其他路徑維持現狀；它們產生的重複資料由上面的合併申請與「建議」
分頁收尾。櫃台在 Z01 新增學員時，若身分證已存在，改為提示「要不要改為連結家庭」。

**開關與回滾**：申請功能跟著 `FAMILY_ACCOUNTS_V1`。關閉時入口全部隱藏，已送出的申請保留不動。
