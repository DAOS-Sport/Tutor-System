...
| # | 訊息 | 判定 | 根因 | 處置 |
|---|---|---|---|---|
| 1 | `[Cron/RagicOutbox] failed: 22P02` | **程式 bug（根因已確認並修，已修）** | **根因：emoji 家長名 + `maskName` 用 `s[0]` 切字**。emoji 是 UTF-16 代理對，`s[0]` 只取半個孤立高代理；`JSON.stringify` 照過，但 `createParentIdentityBackofficeTask` 的 `$2::jsonb` 被 Postgres 22P02 拒收（low surrogate must follow high surrogate）。發生在 `_markFailure` 交易內 → rollback（錯誤碼永遠空）→ 從 catch 再炸出 → cron `break` → 當晚其他 pending 全沒處理。2 筆（07-25/07-31）卡 45 天。dev 用中文假名重現不出，最後對正式站那筆 monkeypatch 實跑抓到 stack | **三層**：①根因 `piiMask.maskName/maskStudentName` 改 `Array.from()` 以 code point 切（測試 `pii_mask_unicode` 7 條＋dev 庫 `::jsonb` 實證，mutation 紅）；②保險 `_markFailureOrQuarantine` 隔離＋印 pg where/table（6 條、mutation 5/5 紅）；③cron 批次層印 where/table |
