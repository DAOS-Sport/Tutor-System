...
> ⚠️ **本文件已部分過時（2026-07-16 註記）**：「簽到／扣課」路線已改版並凍結（見根目錄 `CLAUDE.md` 凍結令與 `replit.md`「簽到／扣課政策（2026-07 改版）」）。本文件中與其衝突的指令——特別是「共享課期拒絕」的手動扣課 e2e 期望——已被新政策取代，**不得依本文件把行為改回去**。其餘 release 流程注意事項仍可參考。
