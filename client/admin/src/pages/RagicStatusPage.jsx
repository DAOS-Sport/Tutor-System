...
import { jobState, reasonText, summarizeFailures } from '../utils/ragicStatusView.mjs';
...
// 「部分完成」：上次失敗的原因全是資料本身的問題（例如每晚寫回 Ragic 時，少數學員缺生日、
// 身分證和其他帳號重複），其他資料照常處理——不能跟整個壞掉的「失敗」共用同一個紅燈。
// 判斷邏輯在 utils/ragicStatusView.mjs（有單元測試）。
function isPartial(info, issues) {
  return info.last_status === 'error' && jobState({ last_status: 'error' }, issues).key === 'partial';
}

function statusBadge(info, issues, inProgress) {
  const base = 'inline-block whitespace-nowrap rounded px-2 py-0.5 text-xs font-bold';
  const s = info.last_status;
  ...
  if (isPartial(info, issues)) return <span className={`${base} bg-amber-100 text-amber-800`}>部分完成</span>;
...
function FormCard({ job, info, schedule, issues, onSync, syncing, isAdmin, envEnabled, onToggle, toggling }) {
  const inProgress = !!info.in_progress || syncing;
  const adminEnabled = info.admin_enabled !== false;
  const canRun = envEnabled && adminEnabled;
  const partial = isPartial(info, issues);
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {statusBadge(info, issues, inProgress)}
          {isAdmin ? (
            <ToggleSwitch
      ...
      <dl className="mt-3 space-y-1.5 text-xs">
        <div className="flex justify-between">
          <dt className="text-gray-500">排程</dt>
          <dd className="text-gray-800">{schedule?.text || '—'}</dd>
        </div>
        <div className="flex justify-between">
      ...
      {info.last_error ? (
        /^unmatched_staff_warning=/.test(info.last_error) ? (
          ...
          <div className="mt-2 rounded bg-amber-50 px-2 py-1.5 text-xs text-amber-800">
            {issues.permanent} 筆資料寫不進 Ragic：
            {issues.reasons.map((r) => `${reasonText(r.code)} ${r.count} 筆`).join('；')}。
            其他資料照常處理{info.last_run_count != null ? `（上次 ${info.last_run_count} 筆）` : ''}。
          </div>
        ) : (
...
export default function RagicStatusPage() {
  ...
  // 寫不進 Ragic 的資料統計（唯讀），用來判斷卡片是「部分完成」還是真的「失敗」；讀不到就照舊顯示
  ...
  async function loadFailures() {
    try { setFailures(await ragicStatusApi.syncFailures(2)); } catch { /* 讀不到就照舊顯示「失敗」 */ }
  }
  ...
  // 「清除錯誤載入資料」按鈕已移除（2026-09-23）：後端 POST /purge-ghosts 早已停用、
  // 一律回 410（破壞性 reconcile 不再允許），按了只會失敗。後端端點保留，讓舊版前端安全失敗。
  ...
  // 排程時間一律用後端 schedules（constants/ragicJobSchedules.js，與 cron/index.js 由測試比對），不寫死
  const sched = (job) => data.schedules?.jobs?.[job]?.text || '—';
  const background = (key) => (data.schedules?.background || []).find((b) => b.key === key)?.text || '—';
  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-2 text-[11px] text-gray-500">
          狀態抓取時間：{fmtDate(data.now)}（各項排程時間見下方卡片）
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {Object.entries(forms).filter(([job]) => !RETIRED_JOBS.includes(job)).map(([job, info]) => (
          <FormCard
            ...
            job={job}
            info={info}
            schedule={data.schedules?.jobs?.[job]}
            issues={issuesByJob[job]}
            onSync={runSync}
      ...
      <div className="mt-6 rounded-lg bg-gray-50 p-4 text-xs text-gray-600">
        ...
        <ul className="mt-1 list-disc space-y-1 pl-4">
          ...
          <li>H01 員工（含教練 1:1 同步）與 H05 場館為定期 <span className="font-bold">全表同步</span>（{sched('staff')}，差異會進待審核區）；Z01 家長 / Z02 學員的 parents/students 卡片是「按請求查詢」<span className="font-bold">健康檢查 Ping</span>，不會抓全表——全表同步由下方「拉回」（{sched('pull')}，Ragic→本地）與「備份」（{sched('backup')}，本地→Ragic）兩張卡片負責，方向相反。</li>
          <li>「部分完成」：上次執行時只有個別資料有問題（例如學員缺生日、身分證和其他帳號重複），其他資料照常處理。</li>
          <li>新註冊的家長{background('outbox')} 寫入 Ragic；Ragic 即時通知處理失敗時{background('webhook')}重試。</li>
