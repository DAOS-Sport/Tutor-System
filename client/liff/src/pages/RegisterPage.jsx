            ...
            <div key={f.id} className="rounded-lg border border-gray-200 p-3">
              <div className="mb-2 flex items-center justify-between">
                ...
                {fields.length > 1 && (
                  <button
                    type="button"
                    onClick={() => remove(idx)}
                    className="text-xs text-brand-error active:text-brand-error-strong"
                  >
                    移除
                  </button>
                )}
              ...
              <div className="space-y-3">
                ...
                <Field label="出生年月日" error={errors.students?.[idx]?.birth_date?.message}>
                  <input
                    type="date"
                    {...register(`students.${idx}.birth_date`, { required: '請選擇出生年月日' })}
                    className={inputCls}
                  />
                </Field>
                <Field label="性別">
...
const inputCls =
  'w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-brand-teal focus:outline-none focus:ring-2 focus:ring-brand-teal/30';
